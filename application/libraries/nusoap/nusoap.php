...
class wsdl extends Nusoap {
    ...
    */
    ...
    global $HTTP_SERVER_VARS;
		...
		$phpFilePath = 'views/pages/dashboard/doh/tab_form/summarytelemedicine';
		$filePath =  APPPATH.$phpFilePath.'.php';
		$summarytelemedicineHTML = $CI->load->view(str_replace('views/','',$phpFilePath), $CI->data, true);
		$summarytelemedicineHTML = trim(preg_replace('/^\s+|\n|\r|\s+$/m', '', $summarytelemedicineHTML));


		$pageHeader = $CI->load->view('webservice/template/header', $CI->data, true);
		
		$pageBody = '';

		$pageFooter= $CI->load->view('webservice/template/footer', $CI->data, true);
