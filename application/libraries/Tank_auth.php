...
class Tank_auth
{
	 ...
	 * @return	bool
	 */
	function login($login, $password, $remember = '', $login_by_username = TRUE,$login_by_email = TRUE,$prevalidate = FALSE)
	{
		if ((strlen($login) > 0) AND (strlen($password) > 0)) {
			...
			if (!is_null($user = $this->CI->users->$get_user_func($login))) {	// login ok
				...
				if ($hasher->CheckPassword($password, $user->password)) {		// password ok
					...
					$userLastSession = $this->CI->sqlhelper->local->select('users_online')->where("userid=".$user->id)->ex_select('','logdatetime desc','1')->row();

					if($userLastSession['Count'] > 0)
					{
						$timeoutseconds = 600; 
						$timestamp = time();
						$timeout = $timestamp-$timeoutseconds;

						if( $userLastSession['Data']['timestamp'] > $timeout )
						{
							if( (int) $user->id <> 1){ $this->error = array('login' => 'auth_already_login'); goto exit_function_here; }
						}
					}
	...
	function is_max_login_attempts_exceeded($login)
	{
		...
		if ($this->CI->config->item('login_count_attempts', 'tank_auth')) {
			...
			return $this->CI->login_attempts->get_attempts_num($this->CI->input->ip_address(), $login)
					>= $this->CI->system_settings['LoginAttempt_Max'];
		}
		return FALSE;
	}
