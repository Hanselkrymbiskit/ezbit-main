...
$DataTableConfig =  array(
        ...
        'column'          =>array(
                                ...
                                array(
                                  ...
                                  'width'              => '200px',
                                  'className'          => 'font-size-10 text-uppercase align-middle',
                                  'visible'            => (@$api_key_filter <> '') ? false : true,
                                  'exportable'         => (@$api_key_filter <> '') ? false : true,
                                  'inputdisplay'       => 'text',
                                  'inputcustomattr'    => '',
                                  'inputcustomclass'   => 'text-uppercase',
                                  'referenceid'        => '',
        ...
        'pushparameter'     =>['api_key_filter' => @$api_key_filter]
      );
...
var vlD = function(logid)
{
  ...
  var PostResult = function(pp,rp){
    if(rp.Status == 1)
    {
      ...
      var PBox = '';
      PBox += '<div class="card border-success mb-0" style="height:90vh;">';
          ...
          PBox += '<div class="nav-tabs-horizontal nav-tabs-inverse" data-plugin="tabs">';
            ...
            PBox += '<div class="tab-content p-0">';
              PBox += '<div class="tab-pane active" id="payload_Request" role="tabpanel">';  
                PBox += '<div class="scrollbar-dark thin" style="background: #646464; border-radius: 5px;overflow-y:auto;max-height:calc(80vh - 160px);">';
                  PBox += '<pre class="mb-0 text-white">'+( (x.aparams) ?  JSON.stringify(JSON.parse(atob(x.aparams)),null,2) : '')+'</pre>';
                PBox += '</div>';
              PBox += '</div>';
              PBox += '<div class="tab-pane" id="payload_Response" role="tabpanel">';
                PBox += '<div class="scrollbar-dark thin" style="background: #646464; border-radius: 5px;overflow-y:auto;max-height:calc(80vh - 160px);">';
                  PBox += '<pre class="mb-0 text-white">'+( (x.aresponseparams) ? JSON.stringify(JSON.parse(atob(x.aresponseparams)),null,2) : '') +'</pre>';
                PBox += '</div>';
              PBox += '</div>';
            PBox += '</div>';
          PBox += '</div>';
    ...
    if( typeof oTable_Obj_Return[DTTableID] == 'object')
    {
      if( oTable_Obj_Return[DTTableID].hasOwnProperty('MiniStats') )
      {
        ...
        for(var st in xms)
        {
          ...
          for(var t in v)
          {
            $("span[id='"+st.toLocaleLowerCase()+"_"+t.toLocaleLowerCase()+"']").html(numberWithCommas(parseInt(v[t])));
          }
        }
      } 
    }
