...
<div id="claim_submit_maincontent" class="row">
	...
	for($ai=1;$ai<=$afcnt;$ai++)
	{
		...
		$fileClick = '';
		$fileuploadid = '';
		if( is_array(@$claimsFormData['attachments']) && isset($claimsFormData['attachments'][$ai - 1]) )
		{
			$fileuploadid = @$claimsFormData['attachments'][$ai - 1]['uploadid'];

			$fileInfo = $CI->sqlhelper->local->select('system_fileupload')->where("DataCount = '".$fileuploadid."'")->row();
			$fileInfo  = $fileInfo['Data'];
			$BlobContent = file_get_contents($fileInfo['File_Path']);
			$pdf = new FpdiProtection('P', 'mm','A4', true);
			if( (int) $CI->userclassification == 1 )
			{
					...
					<div class="row">
						<div class="col-md-10 col-lg-10">
							<div class="form-group form-material form-material-file mb-0" data-plugin="formMaterial">
				                ...
				                <input type="file" id="attachment['.$ai.']" name="attachment" fid="'.@$fileuploadid.'" title="Browse File" forminputgroup="claimFormData" forminputgroupcheck="claimFormData" autocomplete="false" style="" accept="application/pdf">
				                ' : '' ).'
				                '.( ( @$ForCompliance == true && $fileuploadid) ? '<input type="hidden" id="attachment[Replacement_'.@$fileuploadid.']" name="attachment[Replacement_'.@$claimsFormData['attachments'][$ai - 1]['uploadid'].']" forminputgroup="claimFormData" forminputgroupcheck="claimFormData" autocomplete="false" value="">' : '').'
				            </div>
			            </div>
			            <div class="col-md-2 col-lg-2 text-center">
			            	<i id="FileViewer_attachment['.$ai.']" class="fa fa-file-pdf-o font-size-50 '.(($fileClick == '') ? 'invisible' : '').'" role="button" title="View Attachment" '.@$fileClick.'></i>
			            </div>
		            </div>
				</div>
			</div>
		...
		$(this).change(function(){		
		    ...
		    if(file.length > 0)
		    {
			    ...
			    if( jQuery.inArray( file['type'], fileTypes ) < 0 || file['size'] > 10000000 )
			    {
				    ...
				    $(this).val('');
      		    	if( $("input[type='hidden'][id='attachment[Replacement_"+$(this).attr('fid')+"]'][forminputgroup='claimFormData']").length > 0 )
				    {
				    	$("input[type='hidden'][id='attachment[Replacement_"+$(this).attr('fid')+"]'][forminputgroup='claimFormData']").val('');
				    }
			      ...
			      reader.onload = function (e,a,eleID) {
					...
					xfiles.text().then(xy=> {
						...
						else
						{
					        ...
					        if( $("input[type='hidden'][id='attachment[Replacement_"+fid+"]'][forminputgroup='claimFormData']").length > 0 )
						    {
						    	$("input[type='hidden'][id='attachment[Replacement_"+fid+"]'][forminputgroup='claimFormData']").val('Y');
						    }
						}
					});
