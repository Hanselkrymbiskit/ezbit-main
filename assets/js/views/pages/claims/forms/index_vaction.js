function receivedClaim(patitle,case_no,ecase_no)
{
  if(case_no && ecase_no)
  {
    var ihtml = `
    ...
    var rPreAuth = Swal.mixin({
        ...
        title: 'Receive Z-Benefit Claim Form',
        icon:'question',
        html: ihtml,
        ...
        if (result.value)
        {
          ...
          var apiPath = site_url+'claims/index/receivedClaim';
          var apiResult = function(pp,rp){

            if(rp.Status == 1)
            {
              toastr.success("Z-Benefit Claim Form","Pre-Auth Case No : "+case_no+" Successfully received!");
            }
...
function statusClaim(patitle,case_no,ecase_no,astatus)
{
  if(case_no && ecase_no)
  {
    ...
    remarkshtml = `
      <div>
        ...
        <div class="form-group form-material mt-0 mb-0 h-400 w-p100 text-left" data-plugin="formMaterial">
    ...
    var rPreAuth = Swal.mixin({
        ...
        title: 'Update Z-Benefit Claim Status',
        icon:'question',
        html: ihtml,
        width:'1000px',
        showCancelButton: true,
        ...
        if (result.value)
        {
          PreAuthData = new FormData();
          PreAuthData.append('case_no', ecase_no);
          PreAuthData.append('claims_status', parseInt(astatus));
          ...
          var apiPath = site_url+'claims/index/astatusClaim';
          var apiResult = function(pp,rp){

            if(rp.Status == 1)
            {
              toastr.success("Z-Benefit Claim",rp.Message);
            }
